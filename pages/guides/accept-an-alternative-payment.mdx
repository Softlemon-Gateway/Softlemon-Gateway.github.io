...
This guide covers hosted redirect payments through payment sessions, using Paysafecard (`psc`) as the example. eps (`eps`) works the same way with a different `payment_method`, see [Supported methods](#supported-methods). The customer completes the payment on the provider's hosted page instead of entering details on your site, so no card or voucher data ever touches your integration.
...
- For eps, also send `customer.first_name` and `customer.last_name`. Skrill fills them in on the eps page so the customer only has to pick their bank.
...
| Method | `payment_method` | Countries | Currency | What the customer does |
|---|---|---|---|---|
| Paysafecard | `psc` | Most of Europe and more, see [Availability](#availability) | As enabled for your account | Enters the 16 digit PIN of a paysafecard voucher |
| eps | `eps` | Austria | EUR | Picks their Austrian bank and approves the transfer in online banking |

Each method has to be enabled for your account before you can use it. [Ask before you show a method](#ask-before-you-show-a-method) tells you which ones are enabled and whether they can be offered to a given customer. Everything else in this guide applies to both methods: the request, the redirect, the return and the webhooks are the same.
...
Send the customer's browser to `checkout_url`. This is a full page redirect, not an iframe. The customer confirms the payment on the provider's page. For Paysafecard they enter their voucher PIN there. For eps they pick their bank and are sent on to their online banking to approve the transfer.
...
1. **Country.** Each method has a list of countries it can serve. For Paysafecard (`psc`) that is paysafecard's published availability (most of Europe plus, among others, Australia, Canada, Mexico, New Zealand, the United Kingdom and the United States; not, for example, South Africa or Costa Rica). Your account may carry a narrower list. The check runs on `customer.country_code` before the provider is contacted, and a country the method does not serve is refused with HTTP 400 `ERR_PAYMENT_METHOD_NOT_AVAILABLE_IN_COUNTRY`. Nothing is created and the `reference` stays free. If you do not send `customer.country_code` the check is skipped and the provider decides on its hosted page. eps (`eps`) serves customers in Austria only and only in EUR. An eps session in any other currency is refused with HTTP 400 `ERR_PAYMENT_METHOD_NOT_ENABLED` before the provider is contacted.

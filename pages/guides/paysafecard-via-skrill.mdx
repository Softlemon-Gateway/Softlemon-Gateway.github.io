...
Paysafecard (`psc`) is presented to the customer on a hosted checkout page run by Skrill. Your integration never sees the voucher. You open a payment session, send the customer to Skrill and wait for the gateway to tell you the outcome.

## Who is involved

| Party | Role in the flow |
|---|---|
| Customer | Picks Paysafecard on your site and pays on Skrill's hosted page with their voucher. |
...
| Skrill Quick Checkout | The hosted payment page. Presents Paysafecard, collects the voucher, obtains the authorisation and notifies the gateway. |
| Paysafecard | The voucher scheme behind the page. Approves or declines the payment. |
...
<Mermaid chart={`sequenceDiagram
    ...
    C->>S: Enters the voucher and confirms
    S->>P: Authorisation request
    alt Approved
        P-->>S: Approved
...
2. **You create a payment session.** Call `POST /api/v1/payment-sessions` with the amount, currency, your `reference`, `payment_method: "psc"`, your `success_url` and `cancel_url` and the customer's `country_code` and `ip_address`. The response is `pending_redirect` with a `checkout_url`. Details are in [Step 1 of the alternative payment guide](/guides/accept-an-alternative-payment#step-1-create-a-payment-session).
...
4. **The customer pays on Skrill's page.** Skrill presents Paysafecard, the customer enters their voucher PIN and confirms. Skrill asks Paysafecard to authorise the payment and shows the customer the result.

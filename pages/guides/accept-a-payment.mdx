...
2. When the cardholder finishes, the gateway redirects them back to your `auth_url` with query parameters: `?card_verification_id={id}&status={outcome}`, where the outcome is one of the statuses in the table above (`full_auth`, `attempt`, `unavailable` or `failed`).
3. On `full_auth` or `attempt`, use the `card_verification_id` value exactly like the `id` from the frictionless case. On `unavailable`, decide whether to charge without 3DS. On `failed`, show the customer a payment failure and let them try another card.
